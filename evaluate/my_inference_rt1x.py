...
if __name__ == '__main__':
    ...
    # 将观测值转化为一个时间步对象（包含观测值、奖励、终止状态、步骤计数）
    tfa_time_stp = ts.transition(observation, reward=np.zeros((), dtype=np.float32))
    # 初始化policy状态，仅处理单个样本
    policy_state = tfa_policy.get_initial_state(batch_size=1)
    # 依据时间步和状态得到动作
    action = tfa_policy.action(tfa_time_stp, policy_state)

    # 使用真实数据集，获取steps
    datasets = DATASETS[0]
    dataset_dir = 'data'                       # TODO

    builder = tfds.builder_from_directories(builder_dirs=dataset_dir)
    print('step数据:{}'.format(builder.info.features['steps']))
    ds = builder.as_dataset(split='train[:1]')
    ds_iterator = iter(ds)
    # Obtain the step from one episode from the dataset
    episode = next(ds_iterator)
    steps = episode[rlds.STEPS]

    images = []
    for step in steps:
        im = images.fromarray(np.array(step['obseration']['image']))
        images.append(im)
    ...
    for step in steps:
        ...
        tfa_time_stp = ts.transition(observation, reward=np.zeros((), dtype=np.float32))
        # 根据当前状态选择的动作，某一步的策略对象policy_step
        policy_step = tfa_policy.action(tfa_time_stp, policy_state)
        ...
        action_order = ['terminate_episode', 'world_vector', 'rotation_delta',
                        'gripper_closedness_action']

        # 遍历数据，将真实动作和预测动作按照指定顺序和维度记录在响应字典中
        for i, action in enumerate(gt_actions):
            for action_name in action_order:
                for action_sub_dimension in range(action[action_name].shape[0]):
                    ...
                    predicted_action_name_to_values_over_time[title].append(predicted_actions[i][action][action_sub_dimension])
        ...
        # 将图像axis=0轴拆分得到一个图像数据列表，再按水平方向axis=1合并
